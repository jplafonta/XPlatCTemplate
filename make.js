    ...
    for (var i = 0; i < apis.length; i++) {
        ...
        for (var j = 0; j < api.calls.length; j++) {
            ...
            if (call.url === "/Authentication/GetEntityToken") {
                ...
                // In the SDK, we will split this into two APIs, one that can be called via an AuthContext, and one that can be called via
                // a PlayFabStateHandle w/ a SecretKey. The loginCall can be auto generated, the AuthContext call will be implemented manually 
                // as a one off.
                ...
                if (categorizedApi.otherCallsRequireSecretKey) {
                    throw Error("A single API can't require both an AuthContext and a secretKey");
                }
