...
public:
    ...
    using value_type = typename decltype(EntryT::value);
    using size_type = typename entry_vector_type::size_type;

    DictionaryEntryVector() = default;
    DictionaryEntryVector(typename entry_type const* begin, typename entry_type const* end);
...
public:
    ...
    using pointer_type = typename decltype(EntryT::value);
    ...
    template<typename Unused = std::enable_if_t<std::is_constructible_v<typename ValueT, pointer_type>>*>
...
DictionaryEntryVector<EntryT, Alloc>::DictionaryEntryVector(typename entry_type const* begin, typename entry_type const* end)
{
...
template <typename EntryT, typename ValueT, template<typename AllocT> class Alloc> template<typename Unused>
