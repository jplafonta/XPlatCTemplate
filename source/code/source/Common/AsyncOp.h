...
{
    typedef typename std::result_of<Func(ArgT)>::type ContinuationReturnT;
...
private:
    ...
    StdExtra::optional<ResultT> m_result;
...
template<typename T>
AsyncOp<T>::AsyncOp(ResultT&& result) noexcept
...
{
    static_assert(Detail::IsResult<std::result_of<Func(ResultT)>::type> || Detail::IsAsyncOp<std::result_of<Func(ResultT)>::type>, "Continuation functions must return either Result<T> or AsyncOp<T>");
...
{
    static_assert(std::is_void<typename std::result_of<Func(ResultT)>::type>::value, "Final continuation may not return a value");
...
template<typename T>
AsyncOpContext<T>::AsyncOpContext(ResultT&& result)
